...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xed="http://www.mycore.de/xeditor" xmlns:i18n="xalan://org.mycore.services.i18n.MCRTranslation"
  xmlns:dbt="http://www.mycore.de/dbt" exclude-result-prefixes="xsl i18n dbt"
>

  <xsl:include href="copynodes.xsl" />

  <xsl:param name="DBT.Layout.inputSize" />
  <xsl:param name="DBT.Layout.inputWidth" />

  <xsl:variable name="grid-width" select="12" />
  <xsl:variable name="input-size">
    <xsl:choose>
      <xsl:when test="string-length($DBT.Layout.inputSize) &gt; 0">
        <xsl:value-of select="$DBT.Layout.inputSize" />
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:variable name="input-width">
    <xsl:choose>
      <xsl:when test="string-length($DBT.Layout.inputWidth) &gt; 0">
        <xsl:value-of select="$DBT.Layout.inputWidth" />
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="dbt:template[@name='submitButton']">
  ...
  <xsl:template match="dbt:template[@name='cancelButton']">
  ...
  <xsl:template match="dbt:template[contains('textInput|passwordInput|selectInput|checkboxList|radioList|textArea', @name)]" mode="validation">
  ...
  <xsl:template match="dbt:template[@name='textInput']" mode="widget">
    <input type="text" class="form-control input-{$input-size} {@class}" id="{@id}">
      <xsl:apply-templates select="." mode="inputOptions" />
    </input>
  </xsl:template>

  <xsl:template match="dbt:template[@name='passwordInput']" mode="widget">
    <input type="password" class="form-control input-{$input-size} {@class}" id="{@id}">
      <xsl:apply-templates select="." mode="inputOptions" />
    </input>
  </xsl:template>

  <xsl:template match="dbt:template[@name='selectInput']" mode="widget">
  ...
  <xsl:template match="dbt:template[@name='textArea']" mode="widget">
  ...
  <xsl:template match="dbt:template" mode="inputOptions">
  ...
  <xsl:template match="dbt:template" mode="label">
    <label for="{@id}" class="col-md-{$label-width} control-label">
      <xed:output i18n="{@i18n}" />
    </label>
  </xsl:template>

  <xsl:template match="dbt:template" mode="inputTooltip">
    <xsl:if test="@tooltip">
